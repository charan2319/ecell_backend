...
router.post('/hero', upload.single('image'), async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO hero_images (image_url) VALUES ($1)', [req.file.path]);
        res.json({ message: 'Hero replaced', url: req.file.path });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.post('/about-image', upload.single('image'), async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO about_image (image_url) VALUES ($1)', [req.file.path]);
        res.json({ message: 'About image updated', url: req.file.path });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
